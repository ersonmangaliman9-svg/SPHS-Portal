...
function RegisterForm({ onBack }) {
  ...
  const canSubmit = name && idNum && email && pw && pw === pw2 && pw.length >= 6;

  const handleSubmit = async () => {
    setErr("");
    setSubmitting(true);
    const result = await register({ role, name, idNumber: idNum, email, password: pw });
    setSubmitting(false);
  ...
  if (submitted) {
    return (
      <div style={{ textAlign: "center", padding: "20px 0" }}>
        ...
        <h2 className="spnhs-display" style={{ margin: "0 0 6px", fontSize: 20, color: c.ink }}>Account created</h2>
        <p style={{ fontSize: 13.5, color: c.inkSoft, lineHeight: 1.6, marginBottom: 18 }}>
          Thanks, {name.split(" ")[0] || "there"}. Check <strong>{email}</strong> for a confirmation link,
          then log in with your new {role} account.
        </p>
  ...
  return (
    <>
      ...
      {!mismatch && pw.length > 0 && pw.length < 6 && (
        <div style={{ fontSize: 12, color: c.red, marginBottom: 10, marginTop: -6 }}>Password must be at least 6 characters.</div>
      )}
      {err && <div style={{ fontSize: 12.5, color: c.red, marginBottom: 10, padding: 10, borderRadius: 8, background: c.redSoft }}>{err}</div>}
      <Button disabled={!canSubmit || submitting} onClick={handleSubmit} style={{ width: "100%", justifyContent: "center", marginTop: 4 }}>
...
function LoginScreen({ initialMode = "login", onBackToLanding }) {
  ...
  const handleLogin = async () => {
    ...
    setSubmitting(false);
    if (!result.ok) setErr(result.error || "Could not log in. Check your email and password.");
  };
        ...
        <div style={{ background: c.surface, padding: 40 }}>
          ...
          {mode === "login" ? (
            <>
              ...
              <Field label="Email">
                <input value={emailVal} onChange={(e) => setEmailVal(e.target.value)} placeholder="you@sphs.edu.ph" className="spnhs-focus" style={inputStyle(c)} />
              </Field>
              <Field label="Password">
                <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} className="spnhs-focus" style={inputStyle(c)} />
              </Field>
              ...
              {err && <div style={{ fontSize: 12.5, color: c.red, marginBottom: 14, padding: 10, borderRadius: 8, background: c.redSoft }}>{err}</div>}
              <Button disabled={!emailVal || !pw || submitting} onClick={handleLogin} style={{ width: "100%", justifyContent: "center" }} size="lg">
                {submitting ? "Logging in…" : "Log in"}
              </Button>
              <div style={{ textAlign: "center", marginTop: 14, fontSize: 12.5, color: c.inkSoft }}>
                New to SPHS?{" "}
                <button onClick={() => { setSent(false); setErr(""); setMode("register"); }} style={{ background: "none", border: "none", color: c.pine, fontWeight: 700, cursor: "pointer", fontSize: 12.5 }}>
                  Create an account
                </button>
              </div>
            </>
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        loadProfile(session.user).finally(() => setAuthChecked(true));
      } else {
        setAuthChecked(true);
      }
    });
  ...
  const register = async ({ role: r, name, idNumber, email, password }) => {
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        data: { role: r, name, id_number: idNumber },
      },
    });
    if (error) return { ok: false, error: error.message };
    return { ok: true };
  ...
  const ctxValue = {
    c, theme, setTheme, user, login, logout, register, page, setPage,
  ...
  if (!authChecked) {
    return (
      <div style={{ minHeight: "100vh", background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", color: c.inkSoft, fontFamily: "'Inter', system-ui, sans-serif" }}>
        Loading…
      </div>
    );
  }
